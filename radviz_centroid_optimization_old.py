...
class radviz_optimization_old:
	...
	def gradient(self):
		...
		if self.use_parallel:
			pool = Pool()
			result = pool.map(self.partial, range(1, self.M))
			pool.close()
			pool.join()
			return np.concatenate([[0.0], result])
		else:
	...
	def optimize(self, max_iterations = 200, momentum = 0.05, step_size = 0.2, halting_threshold = 1e-5, printing=False):
		...
		for iteration in range(max_iterations):
			...
			self.update_anchors()
			if printing:
				print(iteration, np.linalg.norm(delta), self.centroid_score())
