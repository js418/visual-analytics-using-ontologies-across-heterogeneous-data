...
class radviz_optimization:
	def __init__(self, data):
		self.data = data
		self.normalize_data()
		self.d_sums = np.sum(self.data, axis = 1)
		self.d_sums[self.d_sums==0] = 1
	...
	def partial(self, k):
		...
		if partial_derivative == np.nan:
			print("Partial derivative undefined at k =", k)
			return np.random.random() * 1e-5
		return partial_derivative * self.M / self.N
	...
	def optimize(self, max_iterations = 200, momentum = 0, step_size = 1, halting_threshold = 1e-5, printing=False):
	...
	def centroid_score(self):
		...
		view = self.get_view(scale_view=False)
		return np.sum([np.linalg.norm(r) for r in (view - np.mean(view, axis=0))]) / self.N
	...
	def get_view(self, scale_view=False):
		''' Returns the resulting [x,y] coordinates of the records. This is a Nx2 matrix.
		If scale_view=True, will scale the projection so the point furthest from the centroid touches the perimeter. '''
		view = []
		...
		view = np.vstack(view)
		if scale_view:
			return view * (1 / np.max([np.linalg.norm(x) for x in view]))
		else:
			return view